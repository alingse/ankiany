...
async def read_web_page_content(args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        content_area = soup.find('article') or soup.find('main') or soup.find('div', class_=lambda c: c and ('content' in c or 'article' in c or 'post' in c))
...
async def create_anki_package_from_cards(args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if isinstance(cards_data, str):
        try:
            ...
            return {
                "content": [{"type": "text", "text": f"Error: 'cards' argument must be a valid JSON list. Parse error: {str(e)}"}],
                "is_error": True
            }
    ...
    for card_dict in cards_data:
        ...
        if m_type == 'qa':
            ...
            if len(parts) >= 3:
                ...
                my_note = genanki.Note(model=model_mcq, fields=[question, options, answer])
                my_deck.add_note(my_note)
            else:
